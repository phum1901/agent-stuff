...
async function pickModelForModeUI(
	...
	// Pi 0.85 exposes ModelRegistry to extensions, while ModelSelectorComponent
	// uses the internal model-runtime interface. Adapt the public registry here.
	const modelRuntime = {
		...
		refresh: (options?: any) => ctx.modelRegistry.refresh(options),
...
class PromptEditor extends CustomEditor {
	...
	override renderTopBorder(width: number, hiddenLineCount: number): string {
		...
		if (!mode) return super.renderTopBorder(width, hiddenLineCount);

		const stripAnsi = (s: string) => s.replace(/\x1b\[[0-9;]*m/g, "");
		const visLen = (s: string) => stripAnsi(s).length;

		let label = ` ${formatModeLabel(mode)} `;
		if (label.length > Math.max(0, width - 4)) {
			label = label.slice(0, Math.max(0, width - 4));
		}
		if (label.length === 0) return super.renderTopBorder(width, hiddenLineCount);
		const labelColor = this.modeLabelColor ?? ((text: string) => this.borderColor(text));

		let leftLead = "── ";
		let statusStyled = "";
		const ind = (this as any).workingStatusIndicator as { renderInBorder?: (w: number) => string } | undefined;
		if (ind?.renderInBorder) {
			const maxStatus = Math.max(1, width - leftLead.length - label.length - 2);
			try {
				const rendered = ind.renderInBorder(maxStatus);
				if (rendered && visLen(rendered) > 0) {
					statusStyled = `${rendered} `;
				}
			} catch {
				statusStyled = "";
			}
		}
		if (!statusStyled && hiddenLineCount > 0) {
			leftLead = `─── ↑ ${hiddenLineCount} more `;
		}
		if (!statusStyled && hiddenLineCount <= 0) {
			leftLead = "";
		}

		const leftVis = leftLead.length + visLen(statusStyled);
		const tail = "─";
		const middleLen = Math.max(0, width - leftVis - label.length - tail.length);
		const middle = "─".repeat(middleLen);

		return this.borderColor(leftLead) + statusStyled + this.borderColor(middle) + labelColor(label) + this.borderColor(tail);
	}
